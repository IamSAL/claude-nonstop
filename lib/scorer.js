...
export function pickPreemptAccount(accounts, currentName, options = {}) {
  ...
  return {
    account: best,
    reason: `higher-priority account freed up (now priority ${best.priority}, was on ${currentName} priority ${current.priority}; session: ${best.usage.sessionPercent}%, weekly: ${best.usage.weeklyPercent}%)`,
  };
}
